...
class Customer {
  ...
  removeOrder(order) {
    const orderIndex = this.#cart.findIndex((ord) => ord === order);
    if (orderIndex > -1) {
  ...
  getWhichCanBeSkipped() {
    ...
    this.#cart.forEach((order) => {
      const list = [];
      let totalSumm = this.#cart.reduce((summ, ord) => {
        summ += ord.price;
        return summ;
      }, 0);
      ...
      const listSumm = list.reduce((summ, ord) => {
        summ += ord.price;
        return summ;
      }, 0);

      const theSame = result.find((set) => {
        return (
          listSumm ===
          set.reduce((summ, ord) => {
            summ += ord.price;
...
orders.forEach((order) => customer.addOrder(order));
...
const theMostCost = whichCanBeSkipped
  .sort((first, second) => {
    const firstSumm = first.reduce((summ, order) => {
      summ += order.price;
      return summ;
    }, 0);
    const secondSumm = second.reduce((summ, order) => {
      summ += order.price;
      return summ;
    }, 0);
...
theMostCost.forEach(order => customer.removeOrder(order));
